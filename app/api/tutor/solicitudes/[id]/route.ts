...
export async function GET(_req: NextRequest, context: RouteContext) {
  try {
    ...
    return NextResponse.json(
      { message: "Error al obtener las solicitudes.", detalle: err?.message },
      { status: 500 }
    );
  }
}
...
export async function PUT(req: NextRequest, context: RouteContext) {
  try {
    ...
    // 3) Volver a consultar la solicitud actualizada
    const [rows] = await pool.query(
      `
      SELECT 
        s.id,
    ...
    return NextResponse.json(
      { message: "Error al actualizar la solicitud.", detalle: err?.message },
      { status: 500 }
    );
  }
}
